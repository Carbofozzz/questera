...
- Analyze how well the answer:  is logically correct and justified, accounts for emotions, motives, relationships and consequences, meets task conditions and fits the world’s lore.

Assign the result:  progress_delta based on rules below.

Compute binary score factors (score each field as 0 or 1 only):
The goal_relevance, if user_answer directly addresses previous_task_summary objective score 1, else - 0.
The actionability, if user_answer contains at least one concrete action/decision score 1, else - 0.
The contradiction_to_task, if user_answer clearly opposes or derails previous_task_summary objective score 1, else - 0.
The explicit_refusal_or_gibberish, if answer is empty / meaningless / pure refusal / totally off-topic score 1, else - 0.
